// maheshpatil369/shrinagar/Shrinagar-afb7f82a73dc8b361fa257e85466f81dc57f7f1e/Frontend1/src/pages/AdminDashboard.tsx
...
type ViewingSellerDetails = {
  seller: Seller;
  products: Product[];
  history: any[]; // Define history type more strictly if possible
};
...
export default function AdminDashboard() {
  ...
  const handleViewSeller = async (sellerId: string) => {
    try {
      const details = await getSellerDetailsForAdmin(sellerId); // <-- FIX: Renamed
            ...
            {viewingSellerDetails && (
                <Tabs defaultValue="profile" className="flex-grow flex flex-col overflow-hidden">
                                ...
                                {viewingSellerDetails.history.map(entry => (
                                    <Card key={entry._id}>
                                        <CardHeader>
                                            ...
                                            <CardDescription className="text-base">Changed by: {entry.changedBy.name} ({entry.changedBy.role})</CardDescription>
                                        </CardHeader>
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t mt-auto">
                        <Button variant="destructive" onClick={() => handleSellerStatusUpdate(viewingSellerDetails.seller._id, 'rejected')} className="text-base"><XCircle className="mr-2 h-5 w-5"/>Reject</Button>
                        <Button variant="secondary" onClick={() => handleSellerStatusUpdate(viewingSellerDetails.seller._id, 'suspended')} className="text-base"><ShieldAlert className="mr-2 h-5 w-5"/>Suspend</Button>
                        <Button className="bg-green-600 hover:bg-green-700 text-base" onClick={() => handleSellerStatusUpdate(viewingSellerDetails.seller._id, 'approved')}><CheckCircle className="mr-2 h-5 w-5"/>Approve</Button>
                    </div>
                </Tabs>
            )}
        </DialogContent>
            ...
            <DialogContent className="sm:max-w-3xl">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-base">
                        ...
                        <strong>{viewingProductDetails?.seller?.businessName}</strong>. Status: 
